...
async function displayDayOnMap(steps, mainDestination) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (step.lat && step.lng && !isNaN(step.lat) && !isNaN(step.lng)) {
            ...
            const query = rawLocation.toLowerCase().includes(mainDestination.toLowerCase()) 
                ? rawLocation : `${rawLocation}, ${mainDestination}`;
    ...
    if (resolvedWaypoints.length === 1) {
        ...
        const marker = new google.maps.Marker({
            ...
            label: { text: "1", color: "#0D0B09", fontWeight: "bold", fontSize: "12px" },
            icon: { path: google.maps.SymbolPath.CIRCLE, scale: 14, fillColor: "#D4AF37", fillOpacity: 1, strokeWeight: 2, strokeColor: "#FFFFFF" }
        });
...
function selectActivityOnMap(step, addressQuery, mainDestination) {
    ...
    const placeMarkerAt = (location) => {
        clearMapOverlays();
        const marker = new google.maps.Marker({
            position: location, map: map, title: actName, animation: google.maps.Animation.DROP,
            icon: { path: "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z", fillColor: "#D4AF37", fillOpacity: 1, strokeWeight: 2, strokeColor: "#FFFFFF", scale: 1.8, anchor: new google.maps.Point(12, 22) }
        });
        activeMarkers.push(marker);
        map.panTo(location);
        map.setZoom(13);
    ...
    if (step && step.lat && step.lng && !isNaN(step.lat) && !isNaN(step.lng) && map) {
        ...
        if (!cleanQuery.toLowerCase().includes(mainDestination.toLowerCase())) cleanQuery = `${locationName}, ${mainDestination}`;

        geocoder.geocode({ address: cleanQuery }, (results, status) => {
            if (status === 'OK' && results[0]) placeMarkerAt(results[0].geometry.location);
            else {
                geocoder.geocode({ address: `${actName}, ${mainDestination}` }, (resFallback, statusFallback) => {
                    if (statusFallback === 'OK' && resFallback[0]) placeMarkerAt(resFallback[0].geometry.location);
                });
            }
        });
    }
