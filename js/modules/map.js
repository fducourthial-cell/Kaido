...
hybridStyles.innerHTML = `
    ...
    @media (max-width: 768px) {
        ...
        .itinerary-map-col { height: 100%; width: 100%; position: relative; padding: 0 !important; }
        
        .sticky-map-box { height: 100%; border-radius: 8px; overflow: hidden; position: relative; padding: 0 !important; border: 1px solid var(--border-color); }
        .sticky-map-box h3, .sticky-map-box p { padding: 0 15px; margin-top: 10px; }
        
        #map { height: calc(100% - 60px) !important; width: 100%; border-radius: 0 0 8px 8px; }
        #activity-preview { display: none !important; } /* On cache l'ancien encart bureau */
    ...
    @media (min-width: 768px) {
        .kaido-carousel { display: none !important; } /* On cache le carrousel sur PC */
    }
`;
...
const goldPinIcon = {
    url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 256 256">
            <g transform="translate(1.40659 1.40659) scale(2.81)">
                <linearGradient id="SVGID_4" x1="45" y1="80.71" x2="45" y2="2.88" gradientUnits="userSpaceOnUse">
                    <stop offset="0%" stop-color="#b07908"/><stop offset="100%" stop-color="#c0943a"/>
                </linearGradient>
...
function initGoogleMap(destinationName, lat, lng) {
    ...
    map = new google.maps.Map(mapContainer, {
        ...
        zoomControl: false, // Épuré pour mobile
        styles: mapStyles[currentTheme] || mapStyles.dark
    });
...
function buildSwipeCarousel(waypoints, mainDestination) {
    ...
    container.innerHTML = '';
    container.style.display = ''; // Laissé vide pour que le CSS flex gère l'affichage mobile

    // L'Observer détecte quand on Swipe (uniquement activé sur mobile)
    const observer = new IntersectionObserver((entries) => {
        if (window.innerWidth > 768) return; // Sécurité : pas d'auto-pan sur desktop
        ...
        root: container,
        threshold: 0.6 // S'active quand la carte est au centre à 60%
    });
    ...
    waypoints.forEach((wp, index) => {
        ...
        // Fetch des photos Google
        if (placesService) {
            const query = `${actName}, ${locationName}`;
            placesService.findPlaceFromQuery({ query: query, fields: ['photos'] }, (results, status) => {
...
async function displayDayOnMap(steps, mainDestination) {
    ...
    // Création du carrousel responsive
...
function selectActivityOnMap(step, addressQuery, mainDestination) {
    ...
    // On cache le carrousel
    const carousel = document.getElementById('kaido-map-carousel');
    if (carousel) carousel.style.display = 'none';

    // Remplissage de l'aperçu Desktop uniquement si on est sur un grand écran
